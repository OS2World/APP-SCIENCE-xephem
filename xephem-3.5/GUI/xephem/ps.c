...
XPSClose()
{
	...
	if (xpsc.wantpr) {
	    ...
	    (void) sprintf (msg, "Print command:\n%s", buf);
	    xe_msg (msg, 0);
	    if (system (buf)) {
...
Font fid;
{
	...
	Display *dsp = XtDisplay(toplevel_w);
	int (*oldhandler)() = XSetErrorHandler (IgnoreXError);
...
create_print_w()
{
	...
	XtManageChild (rb_w);
	    ...
	    set_xmstring (gray_w, XmNlabelString, " Grayscale");
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: ps.c,v $ $Date: 2001/10/03 08:28:48 $ $Revision: 1.27 $ $Name:  $"};
