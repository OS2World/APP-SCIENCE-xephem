/* main() for xephem.
 * Copyright (c) 1990-2001 by Elwood Charles Downey
...
extern void sr_chknightv P_((void));
...
extern void wdb_manage P_((void));
...
static void newEnv P_((int *argcp, char *argv[]));
...
typedef enum {
    ...
    PLOT, LIST, SEARCH, GLANCE, AAVSO, SETIAH, CCONV,
    DB, OBJS, CLOSEOBJS, FIELDSTARS, WEBDB,
    ABOUT, REFERENCES, INTRO, MAINMENU, FILEHLP, VIEWHLP, TOOLSHLP, OBJHLP,
	PREFHLP, OPERATION, CONTEXTHLP, SRCHHLP, DATETIME, NOTES
} MBOptions;
...
static char xephem_bits[] = {
 0x00,0x00,0x00,0x00,0x00,0xc0,0x01,0x00,0x00,0xfc,0x1f,0x00,0x00,0xc3,0x61,
 0x00,0xc0,0x00,0x80,0x01,0x20,0x00,0x00,0x02,0x10,0xf0,0x07,0x04,0x08,0x0c,
 0x18,0x08,0x08,0x03,0x60,0x08,0x84,0x00,0xc0,0x11,0x84,0xf0,0xc7,0x11,0x42,
 0x08,0xc8,0x21,0x42,0x04,0x10,0x21,0x22,0x02,0x20,0x22,0x22,0xc2,0x21,0x22,
 0x22,0xe2,0x23,0x22,0x22,0xe2,0x23,0x22,0x22,0xe7,0x23,0x22,0x22,0xc7,0x21,
 0x22,0x22,0x07,0x20,0x22,0x42,0x04,0x10,0x21,0x42,0x08,0x08,0x21,0x84,0xf0,
 0x87,0x10,0x84,0x00,0x80,0x10,0x08,0x03,0x60,0x08,0x08,0x0c,0x18,0x08,0x10,
 0xf0,0x07,0x04,0x20,0x00,0x00,0x02,0xc0,0x00,0x80,0x01,0x00,0x03,0x60,0x00,
...
#if defined(__NUTC__)
	if (strcmp (ServerVendor(XtD), "MicroImages, Inc.")) {
	    printf ("Requires MicroImages MI/X server\n");
	    exit(1);
	}
#endif
}
...
make_main_window ()
{
	...
	static ButtonInfo help_buttons[] = {
		...
		"Introduction", "Introduction...", 0, 0, 'I', m_activate_cb,
							    (XtPointer)INTRO},
	    {"Search all Help text for string",
		"onSearch", "Search...", 0, 0, 'S', m_activate_cb,
							(XtPointer)SRCHHLP},
...
setup_icon ()
{
	...
	get_something (toplevel_w, XmNiconPixmap, (XtArgVal)&pm);
	if (!pm || pm == XmUNSPECIFIED_PIXMAP) {
	    Display *dsp = XtDisplay (toplevel_w);
	    ...
	    set_something (toplevel_w, XmNiconMask, (XtArgVal)pm);
	}
}
...
XtPointer call;
{
	...
	case SRCHHLP:	hlps_manage (); break;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: xephem.c,v $ $Date: 2001/10/10 22:44:03 $ $Revision: 1.61 $ $Name:  $"};
