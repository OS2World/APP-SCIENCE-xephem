...
static void ng_1col_cb P_((Widget w, XtPointer client, XtPointer call));
...
static void ng_settime_cb P_((Widget w, XtPointer client, XtPointer call));
...
ng_create_popup()
{
	...
	set_xmstring (w, XmNlabelString, "Set time to:");
	XtManageChild (w);
...
XtPointer call;
{
	...
	if (ev->xbutton.y < skyh &&
		(op = db_basic(ev->xbutton.y*NOBJ/skyh))->o_type != UNDEFOBJ) {
...
int *xr, *xs;
{
	...
	/* if event occurs on opposite side of midnight as now, redo for its day
	 */
	nowright = (nowx > midx);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: glance.c,v $ $Date: 2001/10/10 05:44:42 $ $Revision: 1.28 $ $Name:  $"};
